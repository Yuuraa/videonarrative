...
# multi_encoding_data함수 삭제

# TODO: vocab 제거하기
def multichoice_encoding_data(args, questions,question_id, video_id, answers, answer_candidates, mode = 'train'):
    tokenizer = BertTokenizer.from_pretrained("kykim/bert-kor-base") # TODO refactor: 조정 가능하게 해볼까?
    ...
    for idx, question in enumerate(questions):
        ...
        for answer in candidates:
            candidate_encoded = tokenizer.encode(answer)
    ...
    for qe in questions_encoded:
        while len(qe) < max_question_length:
            ...
            qe.append(tokenizer.pad_token_id)
    ...
    for ans_cands in all_answer_candidate_encoded:
        for ans in ans_cands:
            while len(ans) < max_answer_candidate_length:
                ...
                ans.append(tokenizer.pad_token_id)
...
# process_question_multiChoices 함수 삭제
...
def process_questions_mulchoices(args):
    ...
    if args.mode in ['train']:
        ...
        answer_token_to_idx = tokenizer.vocab
        ...
        #         if token not in question_answer_token_to_idx:
        #             question_answer_token_to_idx[token] = len(question_answer_token_to_idx)
        question_token_to_idx = tokenizer.vocab              
        ...
        #         if token not in question_answer_token_to_idx:
        #             question_answer_token_to_idx[token] = len(question_answer_token_to_idx)    
        summ_token_to_idx = tokenizer.vocab              
        ...
        #             if token not in question_answer_token_to_idx:
        #                 question_answer_token_to_idx[token] = len(question_answer_token_to_idx)
        script_token_to_idx = tokenizer.vocab
        question_answer_token_to_idx = tokenizer.vocab
        ...
        multichoice_encoding_data(args, train_questions, train_question_id, train_video_id, train_answers, train_answer_candidates, mode='train')
        multichoice_encoding_data(args, val_questions, val_question_id, val_video_id, val_answers,val_answer_candidates, mode='val')
